...
namespace Kutuphane.Business.Classes
{
        ...
        {
            dataSet = new DataSet();
            ogrenci = new Ogrenci(); // Veritabanından nesne oluşturdum

            return ogrenci.listele(); // listele fonksiyonunu çağırıp gönderdim
        }
        ...
        public Boolean guncelle(int id, string ad, string soyad, string ogrenciNo, string tcNo, string telNo)
        {
            ...
            if (ogrenciNo.Length == 9 & tcNo.Length == 11 & telNo.Length == 10)
            {
                ogrenci.guncelle(id, ad, soyad, ogrenciNo, tcNo, telNo); // Gelen verilerle guncelle fonksiyonunu çağırdım
                return true;
            }
            else return false;
        }
        ...
        public Boolean guncelle(int id,int borc)
        {
            ...
            ogrenci.guncelle(id, borc); // Gelen verilerle guncelle fonksiyonunu çağırdım
        ...
        public Boolean guncelle(int id, Boolean KitapAlabilirMi)
        {
            ...
            ogrenci.guncelle(id, KitapAlabilirMi); 

            return true;
        }
        
        public DataSet ara(int id)
        {
            ogrenci = new Ogrenci();
            dataSet = new DataSet();
